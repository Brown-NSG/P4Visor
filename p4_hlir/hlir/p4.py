...
control_entry_points = {}
def get_control_entry_point(hlir, control_flow, program_version=0):
    if control_flow in control_entry_points:
        return control_entry_points[control_flow]

    control_entry_points[control_flow], _ = p4_control_flow_to_table_graph(hlir, control_flow, program_version=program_version)
    print 'LOG|build hlir|validated|control_entry_points:', control_entry_points
    return control_entry_points[control_flow]
...
def p4_validate(hlir, program_version=0):
    """
    TODO: docstring
    """
    ...
    # print'ZP: main-016-001 self:', pprint(vars(hlir))
        ...
        if parse_state not in visited_parse_states:
            ...
            for branch_key, dst in parse_state.branch_to.items():
                if isinstance(dst, p4_control_flow):
                    ...
                    entry_point = get_control_entry_point(hlir, dst, program_version=program_version)
    ...
    # print'ZP: main-016-002 self:', pprint(vars(hlir))
    # return
    if "egress" in hlir.p4_control_flows:
        egress_fn = hlir.p4_control_flows["egress"]
        hlir.p4_egress_ptr = get_control_entry_point(hlir, egress_fn, program_version=program_version)
...
def p4_validate_shadow_metadata_AB(hlir):
    """
    TODO: docstring
    """
    ...
    p4_types = [
        ...
        # hlir.p4_control_flows,
        # hlir.p4_parse_value_sets,
        # hlir.p4_parse_states, ## Error
        # hlir.p4_parser_exceptions,
    ]
    for hlir_dict in p4_types:
        print 'DBG|p4_hlir|hlir|p4:', hlir_dict
        parse_pragmas(hlir_dict)
        for _, p4_object in hlir_dict.items():
            p4_object.build(hlir)
